CREATE FUNCTION [DBO].[valid_email] (
...
select top 1 @BeforeArobase=Item from  dbo.split_string(@EmailAddr, '@');
if len(@AfterArobase) not between 2 and 255
RETURN(0);

--Controle sous-domain pas plus grand que 63
select top 1 @HasDomainTooLong=1 from dbo.split_string(@AfterArobase, '.') where LEN(Item)>63
